...
const defaultContent: Record<EmptyStateVariant, { title: string; description: string }> = {
  "no-messages": {
    ...
    description: "Myaou is waiting! Start a conversation with someone nearby.",
  },
  "no-friends": {
    title: "Looking for friends",
    description: "Explore places nearby to meet new people and make friends.",
  },
  "no-places": {
    title: "No places joined",
    description: "Join a place to start chatting with people nearby!",
  },
  "no-search-results": {
    title: "Nothing found",
    description: "Myaou couldn't find anything matching your search.",
  },
  "profile-incomplete": {
  ...
  switch (variant) {
    ...
    case "no-places":
      ...
      return (
        <svg viewBox="0 0 120 120" className={baseClasses} fill="none" xmlns="http://www.w3.org/2000/svg">
          {/* Map */}
          <rect x="35" y="55" width="50" height="35" rx="2" className="fill-muted stroke-muted-foreground/40" strokeWidth="1.5" transform="rotate(-5 60 72)" />
          <path d="M45 62 L55 75 L65 65 L75 78" className="stroke-primary/50" strokeWidth="1.5" fill="none" strokeDasharray="3 2" />
          <circle cx="75" cy="78" r="3" className="fill-accent" />
          {/* Cat body behind map */}
          <ellipse cx="60" cy="85" rx="20" ry="12" className="fill-cat-pink/20" />
          {/* Cat head */}
          <circle cx="60" cy="40" r="18" className="fill-cat-pink/30" />
          {/* Cat ears */}
          <path d="M45 26 L50 38 L42 38 Z" className="fill-cat-pink/40" />
          <path d="M75 26 L70 38 L78 38 Z" className="fill-cat-pink/40" />
          {/* Cat face - curious expression */}
          <circle cx="53" cy="38" r="2.5" className="fill-foreground/60" />
          <circle cx="67" cy="38" r="2.5" className="fill-foreground/60" />
          <ellipse cx="60" cy="45" rx="2" ry="1.5" className="fill-cat-pink" />
          {/* Paws holding map */}
          <ellipse cx="42" cy="65" rx="5" ry="4" className="fill-cat-pink/30" />
          <ellipse cx="78" cy="65" rx="5" ry="4" className="fill-cat-pink/30" />
        </svg>
      );
